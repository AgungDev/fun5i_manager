...
class AdminAccounts {
	...
	public function login($email, $password){
		...
		$query = $this->db->prepare("SELECT * FROM _admin WHERE email=:email AND sandi=:password LIMIT 1");
	...
	public function getId($token){
		$data = null;
		try{
			$id = $this->isObject($this->jwt->getPayloadResult($token));
			$data = $this->mLib->generate(MessagesLib::$SUCCESS_READ, $id->{"id"});
		}catch(Exception $e){
			$data = $this->mLib->generate(MessagesLib::$TOKEN_INVALID, $e->getMessage());
		}	
	...
	private function isObject($obj){
		if (!is_object($obj)){
			throw new Exception("Bukan object");
		}
	...
	public function getProfile($token){
		...
		if ($this->getId($token)["error"] ){
			...
			$query = $this->db->prepare("SELECT * FROM _admin WHERE id=:id LIMIT 1");
	...
	public function updateFullname($token, $newName){
		...
		try{
			...
			if( $restokid["error"] ){
				...
				$query = $this->db->prepare("UPDATE _admin SET fullname=:fullname WHERE id=:id");
				$query->bindParam(":id", 			$id);
	...
	public function registration($fullname, $email, $password){
		...
		if ( !$this->mLib->errorCheck( $this->checkEmail($email) ) ){
			...
			try{
				$query = $this->db->prepare("INSERT INTO _admin (fullname, email, sandi) VALUES (:fullname, :email, :pass)");
	...
	public function checkEmail($email){
		...
		$query = $this->db->prepare("SELECT fullname FROM _admin WHERE email=:email LIMIT 1");
		$query->bindParam(":email", $email);
